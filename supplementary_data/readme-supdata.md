...
* all\_lake\_coords.csv: list of coordinates for all lakes in the dataset, used in the growth window calculation scripts. Coordinates were collected from the original data files or searched within the database where possible, otherwise they were estimated based on sampling location name.
	...
	* lakes.zip: shapefile (and associated files) showing the lake locations and relevant attributes as well as a QGIS style file that can be used to format the sampling points
	* ssr_stations.zip: shapefile (and associated files) showing the SSR station locations and relevant attributes as well as a QGIS style file that can be used to format the station points
