...
from growth_window_functions import format_lake_data, pci_means, get_coords_ts, get_tsi_coords, \
    select_daily_mean, lake_summary, calc_pci_normalized, format_lake_name, frequency_tsi_climate
import glob
...
gw_data = pci_means(spring_and_summer_doy, spring_and_summer_df,
...
daily_mean.to_csv('output/daily_mean_v2.csv')


lake_summary.drop_duplicates(inplace=True)

lake_list = pci_data >> select(X.lake)
lake_list.drop_duplicates(inplace=True)
lake_list.to_csv('output/lake_list.csv', encoding='utf-8')


# Tasks:


#2) remove lakes and figure out which year for lake winnipeg is the problem
daily_mean = DplyFrame(pd.read_csv('output/daily_mean.csv', encoding='utf-8'))
daily_mean = daily_mean[~daily_mean['lake'].isin(['Kasumigaura', 'Taihu', 'Backcountry lake', 'Eriksundsbron', 'Fulleröfjärden S', 'Hummingbird lake', 'Third Narrows'])]
daily_mean = daily_mean.loc[~((daily_mean['lake'] == 'Ranworth broad') & (daily_mean['year'] == 2006)),:]

len(gw_data.lake.unique())
